...
void TMDQueue_subscribe(TMDQueue *const me, const UpdateFuncPtr updateFuncAddr)
{
    ...
    if (!pNH) /* empty list? */
    {
        /* create a new NotificationHandle, initialize it, and point to it */
    ...
    else
    {
        ...
        printf("----->Adding to an exist list\n");
...
int TMDQueue_unsubscribe(TMDQueue *const me, const UpdateFuncPtr updateFuncAddr)
{
    ...
    {   /* is it the first one */
        ...
        else/* search lists to fine element */
        {
            printf(">>>>>>Searching...\n");
            while (pNH != NULL)
            {
                if (pNH->updateAddr == updateFuncAddr)
                {
                    ...
                    free(pNH);
                    printf(">>>>>>Removing subscribe in list\n");
                    --me->nSubscribers;
                    return 1;
                }/* end if found */
